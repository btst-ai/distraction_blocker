...
chrome.runtime.onInstalled.addListener((details) => {
  ...
  console.log('📥 Extension installed/reloaded:', details.reason);
  ...
  if (details.reason === 'install') {
    ...
    console.log('🔄 Extension reloaded (reason:', details.reason + ')');
    isFirstRun = false;
  }
});
...
async function loadVocabulary(language = null, save = true) {
  ...
  if (state.customDictionaries && state.customDictionaries[lang]) {
    console.log(`📚 Loading custom dictionary: ${state.customDictionaries[lang].name}`);
  ...
  console.log('[LANG-DEBUG] Step 5: loadVocabulary() called');
  console.log('[LANG-DEBUG] Step 5.1: Language parameter:', language);
  console.log('[LANG-DEBUG] Step 5.2: Resolved language:', lang);
  console.log('[LANG-DEBUG] Step 5.3: Filename:', filename);
  
  try {
    console.log('[LANG-DEBUG] Step 5.4: Fetching vocabulary file...');
    ...
    console.log('[LANG-DEBUG] Step 5.5: File fetched. Text length:', text.length);
    ...
    console.log('[LANG-DEBUG] Step 5.6: Vocabulary parsed. Count:', state.vocabulary.length);
    console.log(`📚 Loaded ${state.vocabulary.length} vocabulary words from ${filename}`);
    if (state.vocabulary.length > 0) {
      console.log('[LANG-DEBUG] Step 5.7: First word example:', state.vocabulary[0]);
    }
    
    if (save) {
      console.log('[LANG-DEBUG] Step 5.8: Saving state with new vocabulary...');
      saveState();
      console.log('[LANG-DEBUG] Step 5.9: State saved with vocabulary');
    } else {
      console.log('[LANG-DEBUG] Step 5.8: SKIPPING SAVE (Safe Mode/Init check)');
    }
  } catch (err) {
    console.error('[LANG-DEBUG] ❌ ERROR in loadVocabulary:', err);
    console.error(`❌ Failed to load ${filename}:`, err);
    console.log(`⚠️ Make sure ${filename} exists in the extension directory`);
  }
}
...
chrome.storage.local.get(['state', 'backup_state'], async (result) => {
  ...
  if (result && result.state) {
    ...
    source = 'main';
    console.log('✅ Found saved state in storage');
    ...
    console.log('🛡️ Data Recovery: Extension data was automatically recovered from backup.');
  }

  if (loadedState) {
    isFirstRun = false;
    console.log('📊 Loaded state keys:', Object.keys(loadedState));
    ...
    console.log('📊 Loaded state:', {
      ...
      hasTimeline: state.todayTimeline?.length > 0,
  ...
  if (state.onBreak && state.breakEndTime) {
    ...
    if (now >= state.breakEndTime) {
      console.log('⏰ Break expired while extension was reloaded, ending break...');
      state.onBreak = false;
      ...
      console.log(`✅ Break still active, ends in ${Math.round((state.breakEndTime - now) / 1000 / 60)} minutes`);
    }
  }
  
  // Mark state as loaded - now URL checks can proceed
  stateLoaded = true;
  console.log('✅ State ready - URL blocking now active');
...
function checkNewDay() {
  ...
  if (!state.lastGoalDate) {
    ...
    if (state.dailyGoals && state.dailyGoals.length > 0) {
      console.log('⚠️ Missing lastGoalDate but goals exist - preserving goals and setting date to today');
    ...
    console.log('📅 First run - setting initial date:', today);
    state.lastGoalDate = today;
    ...
    console.log('✅ Same day, keeping goals and timeline');
...
async function startBreak(customDuration = null, unlockCategories = null, unlockAll = false) {
  ...
  if (unlockAll) {
    ...
    console.log('📋 Break whitelist: ALL ALLOWED');
    ...
    if (state.siteCategories) {
      ...
      console.log(`📋 Break whitelist initialized with ${state.breakWhitelist.length} sites from categories:`, unlockCategories);
    } else {
      state.breakWhitelist = [];
    ...
    state.breakWhitelist = [];
    console.log('📋 Break whitelist: NONE (Strict Break)');
  }
...
async function endBreak() {
  ...
  if (wasEndedEarly) {
    ...
    console.log(`⏹ Break ended early after ${actualDuration} minutes`);
...
function extendBreak(additionalMinutes) {
  ...
  console.log(`⏰ Snoozed break by ${additionalMinutes} minutes`);
...
function showBreakWarningNotification() {
  console.log('⚠️ Attempting to show break warning notification');
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'getState') {
    ...
    console.log('⏰ Snooze requested:', duration, 'minutes');
    ...
    if (!alreadyExists) {
      ...
      saveState();
      console.log(`➕ Added site: ${domain} (${category})`);
      sendResponse({ success: true });
    } else {
    ...
    if (!alreadyExists) {
      ...
      saveState();
      console.log('✅ Added whitelisted site:', domain);
      sendResponse({ success: true });
    } else {
    ...
    if (request.index >= 0 && request.index < state.whitelistedSites.length) {
      ...
      saveState();
      console.log('➖ Removed whitelisted site:', removed[0]);
      sendResponse({ success: true });
    } else {
      sendResponse({ success: false, error: 'Invalid index' });
    }
    ...
    console.log('[LANG-DEBUG] Step 4: updateSettings handler called');
    console.log('[LANG-DEBUG] Step 4.1: Request vocabLanguage:', request.vocabLanguage);
    console.log('[LANG-DEBUG] Step 4.2: Current state vocabLanguage:', state.vocabLanguage);
    
    // CRITICAL: Capture sendResponse IMMEDIATELY and return true BEFORE any async work
    // This keeps the message channel open
    console.log('[LANG-DEBUG] Step 4.0.1: Capturing sendResponse...');
    console.log('[LANG-DEBUG] Step 4.0.2: sendResponse type:', typeof sendResponse);
    console.log('[LANG-DEBUG] Step 4.0.3: sendResponse is function?', typeof sendResponse === 'function');
    console.log('[LANG-DEBUG] Step 4.0.4: sendResponse.toString():', String(sendResponse).substring(0, 100));
    ...
    const safeSendResponse = (data) => {
      console.log('[LANG-DEBUG] Step 4.0.5: safeSendResponse called with:', data);
      console.log('[LANG-DEBUG] Step 4.0.6: responseSent flag:', responseSent);
      console.log('[LANG-DEBUG] Step 4.0.7: responseCallback type:', typeof responseCallback);
      console.log('[LANG-DEBUG] Step 4.0.8: responseCallback is function?', typeof responseCallback === 'function');
      
      if (!responseSent) {
        try {
          console.log('[LANG-DEBUG] Step 4.0.9: About to call responseCallback...');
          console.log('[LANG-DEBUG] Step 4.0.9.1: Timestamp before sendResponse:', Date.now());
          const result = responseCallback(data);
          console.log('[LANG-DEBUG] Step 4.0.10: responseCallback returned:', result);
          console.log('[LANG-DEBUG] Step 4.0.10.1: Return type:', typeof result);
          console.log('[LANG-DEBUG] Step 4.0.10.2: Return is true?', result === true);
          console.log('[LANG-DEBUG] Step 4.0.10.3: Return is false?', result === false);
          console.log('[LANG-DEBUG] Step 4.0.10.4: Timestamp after sendResponse:', Date.now());
          ...
          responseSent = true;
          console.log('[LANG-DEBUG] Response sent via safeSendResponse:', data);
          console.log('[LANG-DEBUG] Step 4.0.11: responseSent set to true');
          return true;
        } catch (error) {
    ...
    if (request.vocabLanguage) {
      ...
      console.log('[LANG-DEBUG] Step 4.3: Old language:', oldLanguage, 'New language:', request.vocabLanguage);
      state.vocabLanguage = request.vocabLanguage;
      
      // Reload vocabulary if language changed (async)
      if (oldLanguage !== request.vocabLanguage) {
        console.log('[LANG-DEBUG] Step 4.4: Language changed! Calling loadVocabulary()...');
        // Use IIFE to handle async work
        (async () => {
          try {
            await loadVocabulary(request.vocabLanguage);
            console.log('[LANG-DEBUG] Step 4.5: loadVocabulary() completed. Vocabulary count:', state.vocabulary?.length || 0);
            console.log('[LANG-DEBUG] Step 4.6: Saving state...');
            await saveState();
            console.log('[LANG-DEBUG] Step 4.7: State saved. Sending response...');
            console.log('[LANG-DEBUG] Step 4.7.1: About to call safeSendResponse with:', { success: true });
            safeSendResponse({ success: true });
            console.log('[LANG-DEBUG] Step 4.8: Response sent successfully');
        ...
        console.log('[LANG-DEBUG] Step 4.4.1: Returning true to keep channel open');
        console.log('[LANG-DEBUG] Step 4.4.2: Handler is async?', chrome.runtime.onMessage.hasListener.toString().includes('async'));
        const returnValue = true;
        console.log('[LANG-DEBUG] Step 4.4.3: About to return:', returnValue);
        return returnValue;
      } else {
        console.log('[LANG-DEBUG] Step 4.4: Language unchanged, skipping vocabulary reload');
      }
    } else {
      console.log('[LANG-DEBUG] Step 4.3: No vocabLanguage in request');
    }
    
    // If no async vocabulary reload needed, save state and respond
    console.log('[LANG-DEBUG] Step 4.6: Saving state (no vocab reload)...');
    (async () => {
      try {
        await saveState();
        console.log('[LANG-DEBUG] Step 4.7: State saved. Sending response...');
    ...
    // Return true IMMEDIATELY to keep channel open
    console.log('[LANG-DEBUG] Step 4.6.1: Returning true to keep channel open');
    console.log('[LANG-DEBUG] Step 4.6.2: Handler is async?', chrome.runtime.onMessage.hasListener.toString().includes('async'));
    const returnValue = true;
    console.log('[LANG-DEBUG] Step 4.6.3: About to return:', returnValue);
    return returnValue;

  } else if (request.action === 'resetCooldown') {
    console.log('🔓 Resetting cooldown to 0');
    state.cooldownEndTime = null;
    ...
    if (!alreadyExists) {
      ...
      saveState();
      console.log('🔥 Added to the no go zone:', domain);
      sendResponse({ success: true });
    } else {
    ...
    if (index >= 0) {
      ...
      saveState();
      console.log(`⏳ Started 15-minute removal timer for nogo list: ${domain}`);
    ...
    if (index >= 0) {
      ...
      saveState();
      console.log(`✅ Removed from nogo list and added to blacklist: ${domain}`);
      sendResponse({ success: true });
    } else {
      sendResponse({ success: false, error: 'Site not in nogo list' });
    }
    ...
    if (state.nogoListRemovalTimers && state.nogoListRemovalTimers[domain]) {
      delete state.nogoListRemovalTimers[domain];
      saveState();
      console.log(`❌ Cancelled removal timer for nogo list: ${domain}`);
      sendResponse({ success: true });
    } else {
    ...
    if (!alreadyExists) {
      ...
      saveState();
      console.log(`➕ Added vocabulary word back: ${word} / ${translation} (${transcription})`);
      sendResponse({ success: true });
    } else {
    ...
    if (removed) {
      ...
      saveState();
      console.log(`🗑️ Removed vocabulary word: ${word} / ${translation}`);
      sendResponse({ success: true, removed: true });
    } else {
      console.log(`⚠️ Word not found in vocabulary: ${word} / ${translation}`);
    ...
    saveState();
    console.log(`📚 Created new dictionary: ${newDict.name} (${newDict.vocabulary.length} words)`);
    ...
    if (state.breakWhitelist.length === 0) {
      ...
      if (!alreadyExists) {
        ...
        saveState().then(() => {
          console.log('✅ Added to break whitelist:', domain);
          sendResponse({ success: true });
        }).catch((error) => {
    ...
    if (state.breakWhitelist.length === 0) {
      ...
      saveState().then(() => {
        console.log('➖ Blocked during break (removed from allowed):', domain);
        console.log('📋 Break whitelist now:', state.breakWhitelist);
        sendResponse({ success: true });
      }).catch((error) => {
        console.error('❌ Failed to save state:', error);
        sendResponse({ success: false, error: 'Failed to save state' });
      ...
      if (index >= 0) {
        ...
        saveState().then(() => {
          console.log('➖ Removed from break whitelist:', domain);
          sendResponse({ success: true });
        }).catch((error) => {
          console.error('❌ Failed to save state:', error);
          sendResponse({ success: false, error: 'Failed to save state' });
    ...
    saveState();
    console.log(`✅ Added category '${category}' to break whitelist (${addedCount} sites)`);
    ...
    if (sitesInCat.length === 0) {
      ...
      if (state.breakWhitelist.length === 0) {
        ...
        saveState();
        console.log(`➖ Blocked category '${category}' during break (switched from all allowed)`);
        sendResponse({ success: true });
        return;
      }
...
function checkAndBlockUrl(url, tabId, source) {
  ...
  console.log(`🔍 Checking [${source}]: ${url} | On break: ${state.onBreak}`);
  ...
  if (isInNoGoList) {
    console.log(`   🔥 HELL LIST: ${url} (never unlocked, even during breaks)`);
    ...
    if (state.unlockAll) {
      console.log(`   ✅ Allowed: ${url} (break: unlock all)`);
      return;
    }
    ...
    if (state.breakWhitelist && state.breakWhitelist.length > 0) {
      ...
      if (isInBreakWhitelist) {
        console.log(`   ✅ Allowed: ${url} (in break whitelist)`);
        return; // Allow this site
      } else {
        console.log(`   🚫 Still blocked: ${url} (not in break whitelist)`);
        // Continue to block this site
      }
    } else {
      // Break whitelist is empty AND unlockAll is false
      // This means STRICT break (nothing allowed)
      console.log(`   🚫 Still blocked: ${url} (strict break - nothing allowed)`);
      // Continue to block
    }
  }
  ...
  if (recent && recent.url === urlLower && (now - recent.timestamp) < 2000) {
    console.log(`   ⏭️  Skipping duplicate check (recently blocked)`);
    return;
  }
  ...
  console.log(`   → NoGo List: ${isInNoGoList}, Blocked: ${isBlocked}, Whitelisted: ${isWhitelisted}`);
  ...
  if (isInNoGoList || (isBlocked && !isWhitelisted)) {
    console.log(`🚫 Blocking ${source}: ${url}`);
    ...
    if (blockedDomain) {
      ...
      console.log(`📊 Block stats: ${state.blockStats.totalBlocksToday} total, ${normalizedDomain}: ${state.blockStats.blocksByDomain[normalizedDomain]}`);
    }
...
function showBlockNotification(url, blockedDomain) {
  ...
  chrome.notifications.create(notificationId, {
    ...
    if (chrome.runtime.lastError) {
      ...
      console.log('✅ Block notification created:', createdId);
...
chrome.webNavigation.onCommitted.addListener((details) => {
  ...
  console.log(`📍 onCommitted: ${details.url} (transitionType: ${details.transitionType})`);
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (changeInfo.url && tab.url) {
    console.log(`📍 onUpdated (hash/URL change): ${tab.url}`);
